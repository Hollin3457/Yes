    ...
    public class MainController : MonoBehaviour
    {
        ...
        private void Application_logMessageReceived(string message, string stackTrace, LogType type)
        {
            if (type == LogType.Log || type == LogType.Error || type == LogType.Exception)
            {
                ...
                {
                    SyncContextUtility.UnitySynchronizationContext.Post(s => WriteMessage(msg), null);
                }
            }
        }
        ...
        {
            if (_onlyLogWhenEnabled && !_dialogUI.gameObject.activeSelf)
            {
                return;
            }
        ...
        {
            var pointerBehavior = PointerUtils.GetPointerBehavior<ShellHandRayPointer>(Handedness.Any, InputSourceType.Hand);
            if (pointerBehavior == PointerBehavior.AlwaysOff)
            {
