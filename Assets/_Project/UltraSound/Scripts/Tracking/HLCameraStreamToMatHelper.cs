...
namespace NUHS.UltraSound.Tracking
{
    ...
    public delegate void FrameMatAcquiredCallback(Mat mat, Matrix4x4 cameraToWorldMatrix, CameraIntrinsics cameraIntrinsics);
        ...
        protected virtual void OnFrameSampleAcquired(VideoCaptureSample sample)
        {
            ...
            if (hasInitEventCompleted && frameMatAcquired != null)
            {
                ...
                if (_rotate90Degree)
                {
                    ...
                    frameMatAcquired.Invoke(rotatedFrameMat, cameraToWorldMatrix, cameraIntrinsics);
                }
                else
                {
                    FlipMat(mat, _flipVertical, _flipHorizontal);

                    frameMatAcquired.Invoke(mat, cameraToWorldMatrix, cameraIntrinsics);
                }
            }
        }
